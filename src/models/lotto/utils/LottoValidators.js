...
class LottoValidator {
  ...
  static isValidWinningNumbers(winningNumbers) {
    if (!this.#isvalidLottoLength(winningNumbers))
      throw new WoowaError(LOTTO_ERROR.SIX_NUMBERS);
    if (this.#isDuplicatedLottoNumberExists(winningNumbers))
      throw new WoowaError(LOTTO_ERROR.DUPLICATED_NUMBER_EXISTS);
    ...
    for (let winningNumber of winningNumbers) {
      ...
      if (!this.#isValidLottoNumber(winningNumber))
        throw new WoowaError(LOTTO_ERROR.INVALID_RANGE);
    }
  }
