...
class PurchaseValidator {
    ...
    static isFitOnPurchaseUnit(purchasePrice) {
        if (!this.#isOverPurchaseUnit(purchasePrice)) throw new WoowaError(PURCHASE_ERROR.NOT_OVER_PURCHASE_UNIT);
        if (!this.#isDivisibleByPurchaseUnit(purchasePrice)) throw new WoowaError(PURCHASE_ERROR.NOT_DIVISABLE_BY_PURCHASE_UNIT);
    }
}
